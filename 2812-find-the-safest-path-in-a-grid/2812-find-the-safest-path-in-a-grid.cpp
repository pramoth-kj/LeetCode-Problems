...
public:
  ...
  vector<pair<int,int>>mv = {{0,1},{1,0}};
    ...
    int maximumSafenessFactor(vector<vector<int>>& grid) {
        ...
        // if thief present at end points.
        if(grid[0][0] == 1 or grid[n-1][n-1] == 1) return 0;

        vector<vector<int>>safe(n,vector<int>(n));
        vector<vector<int>>vis(n, vector<int>(n,0));
        // finding the safeness of each cell using bfs.
        
        queue<pair<pair<int,int>, int>>q; // { {i,j}, safeness }.
        // putting all the thieves in queue.
        for(int i=0;i<n;i++){
          for(int j =0;j<n;j++){
            if(grid[i][j] == 1){
              q.push({{i,j},0});
        ...
        // Safe matrix ready: apply binary search.
